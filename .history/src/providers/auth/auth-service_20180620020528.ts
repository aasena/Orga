...
export class AuthService {
  ...
  async nativeGoogleLogin(): Promise<void> {
    try {
      ...
      return response.additionalUserInfo.profile.email;      
    } catch (err) {
      console.log(err);
    }
  }
  ...
  memoId(email): Observable<Object> {
    return this.http
      .post(
        "http://localhost:3000/api/user/memoid",
        {
          email: email
        }
      )
  ...
  memoCreate(id, user, memo, date, time): Observable<Object> {
    return this.http
      .post(
