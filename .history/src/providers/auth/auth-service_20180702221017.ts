...
export class AuthService {
  ...
  async nativeGoogleLogin(): Promise<void> {
    try {
      await this.googlePlus.login({
        webClientId: "877633766779-c0u3h36b749habnjr40jef6sfr55vt98.apps.googleusercontent.com",
        offline: true
      }).then(function (user) {
        ...
        console.log("suca")
          ...
          console.log(JSON.parse(this.email))
  ...
  memoDelete(id_user, id_memo) {
    return this.http
      .post<Memos>(
        "http://orga-app.it/api/user/memo/delete",
        {
          id_user: id_user,
          id_memo: id_memo
        }
  ...
  noteCreate(email, title, note, new_, newCollection, collection) {
    return this.http
      .post<Memos>(
        "http://orga-app.it/api/user/note/create",
        {
          "email": email,
          "title": title,
          "note": note,
  ...
  getNote(id_user, collection) {
    return this.http
      .post(
        "http://orga-app.it/api/user/note",
        {
          "id_user": id_user,
  ...
  noteDelete(id_user, id_note) {
    return this.http
      .post<Memos>(
        "http://orga-app.it/api/user/note/delete",
        {
          id_user: id_user,
          id_note: id_note
        }
  ...
  noteEdit(id_user, id_note, note, title, collection, new_, new) {
    return this.http
      .post<Memos>(
        "http://orga-app.it/api/user/note/edit",
        {
          id_user: id_user,
          id_note: id_note,
          note: note,
